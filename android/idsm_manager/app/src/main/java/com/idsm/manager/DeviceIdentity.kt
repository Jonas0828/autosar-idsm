...
object DeviceIdentity {
    ...
    val manufacturer: String
        get() = SysProps.get("persist.idsm.manufacturer", "caic")

    val modelCode: String
        get() = SysProps.get("persist.idsm.model_code", "t99")

    val vin: String
        get() = SysProps.get("persist.idsm.vin", "UNKNOWN_VIN")

    /** 三段式设备编号 */
    fun deviceId(): String = "${manufacturer}_${modelCode}_${vin}"

    /** 本机 ECU 编码(8.2 数据字典, 默认 0x02 座舱主机) */
    fun ecuCode(): String = SysProps.get("persist.idsm.ecu_code", "0x02")

    /** 本机托管的探针节点(属性上报覆盖范围, 8.1) */
    fun nodeTypes(): List<String> =
        SysProps.get("persist.idsm.nodes", "HIDPS,NIDPS")
