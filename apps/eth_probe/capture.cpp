...
bool LiveCapture::open(const std::string& ifname, std::string& err) {
    fd_ = ::socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
    if (fd_ < 0) {
        err = "socket(AF_PACKET): " + std::string(std::strerror(errno)) +
              " (root required)";
        return false;
    }
...
bool LiveCapture::run(const FrameCallback& cb, std::string& err) {
    uint8_t buf[65536];
...
bool replay_pcap(const std::string& path, const FrameCallback& cb,
    ...
    auto rd16 = [swap](const uint8_t* p) -> uint16_t {
        uint16_t v;
        std::memcpy(&v, p, 2);
        if (swap) v = static_cast<uint16_t>((v >> 8) | (v << 8));
        return v;
    };
    ...
    if (linktype != 1) {  /* LINKTYPE_ETHERNET */
        err = "unsupported pcap link type (expected Ethernet=1, got " +
